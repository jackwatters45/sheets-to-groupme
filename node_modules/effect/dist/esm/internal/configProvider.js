...
export const fromEnv = config => {
  ...
  const makePathString = path => pipe(path, ReadonlyArray.join(pathDelim));
    ...
    case OpCodes.OP_SEQUENCE:
      {
        return pipe(pathPatch.patch(prefix, flat.patch), core.flatMap(patchedPrefix => pipe(flat.enumerateChildren(patchedPrefix), core.flatMap(indicesFrom), core.flatMap(indices => {
          ...
          return pipe(core.forEachSequential(indices, index => fromFlatLoop(flat, ReadonlyArray.append(prefix, `[${index}]`), op.config, true)), core.map(chunkChunk => {
    ...
    case OpCodes.OP_HASHMAP:
      {
        return core.suspend(() => pipe(pathPatch.patch(prefix, flat.patch), core.flatMap(prefix => pipe(flat.enumerateChildren(prefix), core.flatMap(keys => {
          return pipe(keys, core.forEachSequential(key => fromFlatLoop(flat, concat(prefix, ReadonlyArray.of(key)), op.valueConfig, split)), core.map(values => {
            if (values.length === 0) {
              return ReadonlyArray.of(HashMap.empty());
            }
            const matrix = values.map(x => Array.from(x));
            return pipe(transpose(matrix), ReadonlyArray.map(values => HashMap.fromIterable(ReadonlyArray.zip(ReadonlyArray.fromIterable(keys), values))));
          }));
        })))));
      }
    case OpCodes.OP_ZIP_WITH:
      {
        return core.suspend(() => pipe(fromFlatLoop(flat, prefix, op.left, split), core.either, core.flatMap(left => pipe(fromFlatLoop(flat, prefix, op.right, split), core.either, core.flatMap(right => {
          ...
          if (Either.isRight(left) && Either.isRight(right)) {
            ...
            const [lefts, rights] = extend(fail, fail, pipe(left.right, ReadonlyArray.map(Either.right)), pipe(right.right, ReadonlyArray.map(Either.right)));
            return pipe(lefts, ReadonlyArray.zip(rights), core.forEachSequential(([left, right]) => pipe(core.zip(left, right), core.map(([left, right]) => op.zip(left, right)))));
          }
...
const splitIndexInKeys = (map, unmakePathString, makePathString) => {
  ...
  for (const [pathString, value] of map) {
    const keyWithIndex = pipe(unmakePathString(pathString), ReadonlyArray.flatMap(key => Option.match(splitIndexFrom(key), {
